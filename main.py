...
parser.add_argument(
    ...
    type=int,
    default=20,
    help='Evaluates a policy a policy every N episode')
parser.add_argument(
    '--gamma',
    type=float,
    default=0.99,
...
parser.add_argument(
    '--comet'
)  # comet.ml login in the format workspace/project/apikey (including the '/' separators)
...
args.seed += 100 #IDK why, but SA-C does the same
...
at_string = "autotune" if args.automatic_entropy_tuning else "fixed_alpha"
filename = f"{datestring}_SAC_{args.env_name}_{args.policy}_{at_string}"
